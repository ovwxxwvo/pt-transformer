...
# 加载配置（读取paths.toml中的日志路径，无则用默认）
config = load_config()
log_path = config.get("paths", {}).get("log_path", "./logs")
os.makedirs(log_path, exist_ok=True)  # 确保日志目录存在

# 自定义日志过滤器：注入版本号（后续对接version.py）
class VersionFilter(logging.Filter):
    def filter(self, record):
        # 先占位，后续导入version.py的__version__替换此处
        record.version = "25.12.01"  # 临时版本，后续改：from utils.version import __version__
        return True
